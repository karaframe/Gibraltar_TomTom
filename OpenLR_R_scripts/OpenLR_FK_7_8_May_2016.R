...
# traffic <- read.csv("traffic_flow_FK.csv") #1-2 May 2016
traffic <- read.csv("traffic_flow_FK_7_8_May_2016.csv")   # 7-8 May 2016
head(traffic)
...
write.csv(POINTS_1st, "points_traffic_1st_7_8_May.csv")
...
write.csv(POINTS_2nd, "points_traffic_2nd_7_8_May.csv")
...
#    content <- paste(sprintf('<h2><strong> hour %02d:00',i),'<h2>', sep = "")
    ...
    map <- leaflet() %>%
      ...
      addLegend("bottomright", pal = pal_NO2, values = c(MIN_NO2, MAX_NO2),
                title = "<strong><p><i>7_8_MAY_2016</i></p><strong><br>Mean NO<sub>2</sub> (<font face=symbol>m</font>g/m<sup>3</sup>): </strong><br>(large circles)",
                labFormat = labelFormat(prefix = ""),
                opacity = 0.6) %>%
      addLayersControl(
